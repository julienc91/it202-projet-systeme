...
/*
 * Fonctionnement : yield() envoie vers le sommet de la file
 */
...
//~ 
	...
	yield_begin:
		...
		else //si toutes les listes sont vides
		{
				...
				for (i = 0; i< nb_cores; i++)
				{
					if(threadList.currentThreads[i] != NULL && threadList.currentThreads[i]->state != DEAD && get_id_thread() !=  (long) i)
					{
						yield_again = TRUE;
						break;
					}
